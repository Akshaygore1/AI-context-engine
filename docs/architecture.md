...
The backend calculates confidence and source labels. A generation catalog exposes only credentialed, allowlisted real providers plus the always-available deterministic mock. It validates a requested provider/model pair before any context read, then resolves real `provider:model` identifiers through the Vercel AI SDK provider registry. The selected generator receives only selected context, safe normalized preferences, the question, and missing-context notes. It returns answer text and operational usage metadata. It cannot redefine confidence or sources, and real failures never fall back to mock.
...
Typed in-code constants provide deterministic mock source responses without persistence or startup initialization. The separate in-memory LRU cache models short-lived upstream reuse and remains independent from fixture storage. It is simple and safe for one process; production replicas would need an external cache if shared hit rates or invalidation mattered.
...
Structured logs correlate the request route/status/total latency with upstream attempts, latency, cache outcome, unavailable services, selected-context count, provider/model identifiers, prompt size, generation latency, and token usage. Logs omit full questions, selected values, prompts, profile details, credentials, base URLs, and secrets. The mock context router has no connection lifecycle or persisted state to initialize or close.
