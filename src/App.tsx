  ...
  return <main>
    ...
    <div className="workspace">
      ...
      <div className="results">
        ...
        {debug && <Card><div className="result-head"><span className="section-label">Independent context inspection</span><span className="mode">{debug.intents.join(" + ")}</span></div><div className="debug-grid"><div><small>Selected</small>{debug.selectedContext.map((item) => <p key={item.id}><strong>{item.label}</strong> · {item.priority}</p>)}{debug.missingContext.length > 0 && <><small>Missing expected context</small>{debug.missingContext.map((item) => <p className="limitation" key={item.id}><strong>{item.label}</strong> · {item.reason}</p>)}</>}{debug.budgetOmissions.length > 0 && <><small>Budget omissions</small>{debug.budgetOmissions.map((item) => <p key={item.id}><strong>{item.label}</strong> · {item.reason}</p>)}</>}</div><div><small>Preferences</small><p>{debug.language} · {debug.tone} · {debug.maxWords} words</p><small>Why</small><p>{debug.reasons.join(" ")}</p>{debug.unavailableSources.length > 0 && <><small>Unavailable services</small><p className="limitation">{debug.unavailableSources.map((item) => `${item.source}: ${item.reason}`).join(" · ")}</p></>}<small>Deliberately excluded</small><p>{debug.excludedContext.map((item) => item.label).join(" · ") || "None"}</p></div></div></Card>}
      </div>
    </div>
  </main>;
}
