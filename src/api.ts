...
export class ApiError extends Error { constructor(message: string, public readonly requestId?: string) { super(message); } }

export async function postApi<T>(path: string, payload: { userId: string; question: string }): Promise<T> {
  const response = await fetch(path, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(payload) });
  const body = await response.json();
  if (!response.ok) throw new ApiError(body.error?.message ?? "The request could not be completed.", body.error?.requestId);
  return body as T;
}
