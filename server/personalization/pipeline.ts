...
const catalog = (data: GatheredContext): Record<string, Omit<ContextItem, "priority">> => ({
  ...
  "kundli.summary": { id: "kundli.summary", label: "Kundli Summary", source: "kundli", value: `Moon sign: ${data.kundli.moonSign}; current dasha: ${data.kundli.currentDasha}` },
});
...
export class PersonalizationPipeline {
  ...
  run(question: string, data: GatheredContext): PersonalizationResult {
    ...
    for (const [id, priority] of ordered) {
      ...
      if (contextCharacters + item.value.length > config.contextMaxChars) {
    ...
    return {
      ...
      decision: {
        ...
        reasons: [`Matched ${detected.intents.join(", ")} using configured weighted phrases.`, "Combined and deduplicated all matched-intent fields, with primary context ordered first."],
        confidence: budgetOmissions.length === 0 ? "HIGH" : context.some((item) => item.priority === "primary") ? "MEDIUM" : "LOW",
        contextCharacters,
      },
    };
  }
