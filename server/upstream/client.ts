...
const kundliSchema = z.object({
  ...
  houses: z.record(z.string(), z.string()),
  ascendant: z.string(),
  planets: z.record(z.string(), z.string()),
});
...
async function resilientRead<T>(
  ...
  if (cached) {
    ...
    console.info(
      JSON.stringify({ event: "upstream", requestId, source, ...outcome }),
    );
  ...
  for (let attempt = 1; attempt <= config.upstreamMaxAttempts; attempt += 1) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        try {
          ...
          lastReason = "Upstream returned malformed JSON.";
          break;
        }
        const parsed = schema.safeParse(payload);
        if (!parsed.success) {
          lastReason = "Upstream returned an invalid payload.";
          break;
        }
    ...
    if (attempt < config.upstreamMaxAttempts)
      await wait(config.upstreamRetryBackoffMs * attempt);
  }
  const outcome = {
    available: false,
    attempts,
...
const endpoint = (
  ...
  if (encodedUser && override.includes("{userId}"))
    return override.replaceAll("{userId}", encodedUser);
...
export async function gatherContext(
  ...
  const requestedProfileSchema = profileSchema.refine(
    (profile) => profile.id === userId,
    { message: "Profile belongs to a different user." },
  );
  const requestedKundliSchema = kundliSchema.refine(
    (kundli) => kundli.userId === userId,
    { message: "Kundli belongs to a different user." },
  );
  const requestedHoroscopeSchema = horoscopeSchema.refine(
    (horoscope) => horoscope.userId === userId,
    { message: "Horoscope belongs to a different user." },
  );
  const [profile, kundli, horoscope, panchang] = await Promise.all([
    resilientRead(
      "profile",
      `profile:${userId}`,
      endpoint("profile", `/users/${encodedUser}`, encodedUser),
      requestedProfileSchema,
      requestId,
    ),
    resilientRead(
      "kundli",
      `kundli:${userId}`,
      endpoint("kundli", `/kundli/${encodedUser}`, encodedUser),
      requestedKundliSchema,
      requestId,
    ),
    resilientRead(
      "horoscope",
      `horoscope:${userId}`,
      endpoint("horoscope", `/horoscope/${encodedUser}`, encodedUser),
      requestedHoroscopeSchema,
      requestId,
    ),
    resilientRead(
      "panchang",
  ...
  return {
    ...
    outcomes: {
      ...
      kundli: kundli.outcome,
      horoscope: horoscope.outcome,
      panchang: panchang.outcome,
    },
  };
}
